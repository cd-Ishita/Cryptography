...
string decimaltohex(int64 deci)
{
    ...
    // Traverse the string EQBIN to find it's equivalent hexa decimal
...
string SHA512(string myString)
{
    ...
    // Stores the string in the fixedstream
    ...
    // Append tobeadded-129 zeros in the string
    for (int y = 0; y < tobeadded - 129; y++) {
        s1024 += "0";
    }
 
    // Stores the binary representation of string length
    string lengthbits= std::bitset<128>(orilen).to_string();
    s1024 += lengthbits; // Append the lengthbits to string
    ...
    for (int letsgo = 0;letsgo < blocksnumber;++letsgo) 
    {
        ...
        // Update the value of A, B, C, D, E, F, G, H
        A += AA;
        B += BB;
        C += CC;
        D += DD;
    ...
    // Print the hexadecimal value of strings as the resultant SHA-512
